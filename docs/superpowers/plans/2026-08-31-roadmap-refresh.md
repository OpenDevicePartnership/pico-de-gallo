...
- **`## Should the RP2350 Stay?` is NOT accurate as written.** An earlier draft of this plan asserted it should be preserved verbatim. That was wrong — it carries two references to protocol sniffing, which was dropped in #16: the PIO row of *The Case For RP2350* lists sniffing as an enabled capability, and *The USB Full Speed Question* contains a whole `**For protocol sniffing:**` bandwidth paragraph analysing a feature that no longer exists. Task 7 fixes both. Everything else in that section was checked and does still hold.

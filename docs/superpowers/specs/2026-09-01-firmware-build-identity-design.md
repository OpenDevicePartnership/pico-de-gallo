...
| # | Risk | Disposition |
|---|---|---|
| R1 | A **new host** talking to **released firmware 0.11.0** never gets a `device/info` answer at all. Appending `build_id` changed `DeviceInfo`'s schema, and postcard-rpc derives each endpoint's key from the response type's schema (`Key::for_path::<T>(path)`, `RESP_KEY` in the `endpoints!` macro), so the endpoint is silently re-keyed. The firmware replies under the old key, the new host's dispatcher finds no match, drops the frame, and never wakes the pending call. `validate()` cannot warn, because both sides still report schema 0.7. | **Accepted and unavoidable** for any shape change to an already-released type — this is broader than appends. It is **not** a decode error: postcard is never reached, so `map_validate_error` never sees `DeserFailed`. `validate()` surfaces it as `ValidateError::Timeout` after the full `DEVICE_INFO_TIMEOUT` (300 s), misattributed to an unresponsive board; the host cannot distinguish the two, and the remediation text now says so. `gallo version` bounds its `device/info` call at 5 s and falls back to the legacy `version` endpoint, whose `VersionInfo` schema — and therefore key — is unchanged. `device_info_response_key_is_pinned` in `pico-de-gallo-internal` now makes any future re-key a test failure instead of a field hang. Host and firmware must be built from the same tree until the 0.8.0 release. Stated in the PR body and the CHANGELOG entries. |

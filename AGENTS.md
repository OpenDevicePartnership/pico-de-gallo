...
`main` at `fcb38de3a1ff` gave 141 and 47, for a pre-#186 total of 693.
Re-measure before trusting these numbers rather than deriving a delta
from them.
...
All 24 ignored tests are board-attached and therefore never run in CI:
...
- The 17 in `pico-de-gallo-lib` are the #135 zero-length-write
  regression tests, the #179 batch-response-ceiling tests, the #158
  per-call-ceiling tests, and the #186 batch-request-frame tests. They
  need one board, and `empty_batch_write_never_reaches_the_bus` and
  `oversized_i2c_batch_never_reaches_the_bus` additionally need a
  TMP102-like target on the I2C bus. The #186 tests additionally need
  **nothing at `0x50`**, which they address deliberately so a batch that
  escaped the guard would be NACKed rather than driven into a real chip.
  Run them with `cargo test -p pico-de-gallo-lib -- --ignored`.

  All 17 were executed on board `49742081C885AC69` on 2026-09-09 during
  the #186 verification and passed in 0.44 s. That run was the **first
  ever execution of the three #158 tests**, which had been written blind
  and were marked as never executed; the in-source note to that effect
  has not been removed, because it still records how they came to exist.
...
| 2026-09-09 | `i2c/batch` or `spi/batch` with `Read` (and, for SPI, `Transfer`) operations totalling 1015..=4096 bytes. Reachable from every host surface, and from the `gallo` CLI, which is *not* accidentally safe here the way it was for #135. | The batch was accepted, **executed on the bus**, and only then lost its response, because a postcard-rpc response frame cannot carry more than 1014 payload bytes. The caller got `Comms(Postcard(DeserializeUnexpectedEnd))`, which reads like a transport fault, says nothing about device state having already changed, and invites the one recovery that makes it worse: a retry, repeating every `Write`. Not a wedge — the board stayed responsive throughout, so this is unlike the 2026-06-03 / 2026-08-19 / 2026-08-26 dispatcher rows. The failure shape was also internally inconsistent: 4097 was refused before any bus activity while 1015 was not. **Two derivation traps.** #158 modelled the edge as `8-byte header + 2-byte varint + 1014 = 1024`, which lands on the right number for the wrong reasons: the header is 7 bytes and the missing byte is the `Result` discriminant that every response-bearing endpoint carries. And **a hand-rolled probe measures a different ceiling than the real client**: probing with a `VarSeq::Seq2` request header measured a 5-byte header and a 1016-byte edge, contradicting the hardware. `HostClient::send_resp` always emits `VarSeq::Seq4` and the server echoes it, and the `VarSeqKind` this repo passes to `try_new_raw_nusb` never takes effect because `VarSeq::resize` is defined in postcard-rpc 0.12.1 and called nowhere in it. A probe must reproduce the client's header exactly or it measures a protocol nobody speaks. | Added `MAX_RESPONSE_PAYLOAD` (1014) to `pico-de-gallo-internal`, derived term by term from the transport rather than fitted, with each term pinned by a test and the two-byte varint plus the response-versus-request distinction pinned by `const` assertions. Both firmware batch handlers now bound `total_read` against it instead of `MAX_TRANSFER_SIZE`, during pre-validation so the refusal precedes any bus activity or chip-select edge, and `pico-de-gallo-lib` pre-validates both entry points so the refusal is local. `BufferTooLong` already existed on both error types, so no wire shape moved and no schema bump was needed. `failed_op = 0`, because an aggregate overflow names no single operation. **Hardware-verified 2026-09-09 on board `49742081C885AC69` (hw-rev2, TMP102 at `0x48`, GPIO 0 as the SPI chip-select witness).** A/B between control image `firmware-v0.11.0-75-gcbf9a6eddb74` and fixed image `firmware-v0.11.0-76-gef33a97918bd`. Control, with the host guard deliberately stubbed out so the firmware is exercised alone: `i2c_batch(0x48, [Write 0x03, Read 1015])` returned `DeserializeUnexpectedEnd` and moved the target's pointer register from TLOW `4b 00` to THIGH `50 00`, so the write was committed; `spi_batch(0, [Read 1015])` returned the same error and moved GPIO 0 from low to high, so chip-select was asserted and deasserted; and the two new `#[ignore]`d refusal tests failed on exactly that error. Fixed image, still driven by a host binary built *without* the guard, so the firmware refusal is what is being measured: both return `BufferTooLong { failed_op: 0 }`, both witnesses are unchanged, 1013 and 1014 still succeed on both buses, 1015..=4097 are refused uniformly, and all 8 board-attached tests pass. **Four limits.** The tests cannot show *which* side refused, since host and firmware return the identical value on purpose; locality is proven only by the `count(...) == 0` scripted-transport assertions. The ceiling is a property of the host's postcard-rpc build that the firmware cannot observe, so it is over-restrictive if that private constant ever grows. Only the *batch* endpoints were bounded: `spi/read`, `spi/transfer`, `i2c/read`, `uart/read` and `onewire/read` still accept up to `MAX_TRANSFER_SIZE` and still lose their response above 1014 — measured again here, `gallo spi read --count 1015` still returns `DeserializeUnexpectedEnd` — which is deliberate scope, since a read commits nothing; that belongs to #158. And the derivation assumes the dispatcher's two-byte key, which `reply_key_length_is_two_bytes` pins but which enough new endpoints would change, dropping the ceiling to 1012. Issue #179, split from #158. |
| 2026-09-09 | Any payload-carrying host call above its real ceiling, on every host surface. Two distinct shapes. **(a)** A read count of 1015..=65535 on `i2c/read`, `spi/read`, `spi/transfer`, `uart/read`, `onewire/read` or the read half of `i2c/write-read` — the sibling of the 2026-09-09 row above, which fixed only the *batch* endpoints. **(b)** `gallo i2c read --count 65537` and its three siblings on the CLI. | **(a) is a diagnosability defect, not a corruption one**, and this is the whole reason it was split from #179: a plain read commits no writes, so nothing is left changed. The caller got `Comms(Postcard(DeserializeUnexpectedEnd))`, which names the transport rather than the argument at fault and suggests no size that would work. Five doc comments in `pico-de-gallo-lib` and four in `pyco-de-gallo` meanwhile promised that an over-limit transfer "will be rejected", which nothing did — and they were wrong twice over, because for a read path the real ceiling is 1014, not the 4096 they stated. **(b) is a silent wrong answer.** The four CLI read counts were `usize` cast with `as u16`, so `--count 65537` became 1: the command asked for 65537 bytes, exited 0, and printed one. No lower layer could catch it, because by the time `pico-de-gallo-lib` saw the value it was an already-legal 1. **The trap worth generalising: a guard placed below a narrowing cast cannot see what the caller actually asked for.** The FFI was checked for the same defect and did *not* have it — all six `usize` entry points already tested `len > u16::MAX` before narrowing — so the initial read of the code, which assumed FFI and CLI shared the bug, was wrong. **A second trap: `MAX_TRANSFER_SIZE` is not one ceiling.** The request and response budgets are independent, measured in #158 by showing a 1021-byte request payload leaving the response ceiling unmoved. `spi/write` completes at 1015 bytes where `spi/transfer` fails, so any single scalar is wrong in one direction. | Two predicates in `pico-de-gallo-lib` — `response_len_is_undeliverable` (generalised from #179's `batch_read_total_is_undeliverable`, since the same bound governs plain reads) and `request_payload_is_too_long` — applied to all thirteen payload-carrying entry points before transmission, per the #136 extracted-policy pattern. A `const` assertion pins the response ceiling as strictly tighter than the request ceiling, which is what lets `spi_transfer` check only one; inverting it was confirmed to be a build failure. Firmware `spi_write_handler` and `i2c_write_handler`, the only two handlers with no upper bound at all, now match the other six. Propagated outward: `UartHalError::kind()` reports `InvalidInput` (I2C and SPI keep `Other`, because `embedded_hal`'s `ErrorKind` has no variant for an oversized argument and `Overrun` would falsely imply a transfer occurred — now pinned by tests so it is a decision on record); the FFI gained `GALLO_MAX_RESPONSE_PAYLOAD`, **verified to reach `pico_de_gallo.h`**, since cbindgen silently drops a const whose initializer is not a literal (§8); `gallo-mcp` validates locally so an agent gets the size, the limit and the remedy instead of `device error: buffer exceeds firmware limit`; the CLI counts became `u16`. No new `Status` value (§8), no wire change, no schema bump, no version bump — `BufferTooLong` already existed on all four peripheral error types. Zephyr's `PDG_SPI_MAX_BUFFER` moved 1013 -> 1014 and into `pdg_spi_bottom.h`, tied to `GALLO_MAX_RESPONSE_PAYLOAD` by a `_Static_assert` in the one translation unit that sees both — the "one shared contract rather than a constant duplicated per consumer" its own FOLLOW-UP comment asked for, which turned out to need no wire change at all. **NOT HARDWARE-VERIFIED.** No board was available; every figure here is inherited from the #158 triage on `5256657D8A5D7F03` (`firmware-v0.11.0-67-g62dd64e710fd`) and the #179 A/B on `49742081C885AC69`, and nothing was re-measured. Verification was static: fmt, clippy `-D warnings`, the full host suite (695 passing, 18 board-attached ignored), `cargo hack --feature-powerset` on `pico-de-gallo-hal` across all six combinations, both firmware revisions, `mdbook build`, and a real `gcc` compile of `pdg_spi_bottom.c` against the generated header. Both new `const`/`_Static_assert` invariants were mutation-tested to confirm they fail when violated. Three board-attached `#[ignore]`d tests were added as the harness for the outstanding A/B and are marked in-source as never executed. **Limits.** The response ceiling is a property of the host's postcard-rpc build, so it is over-restrictive if that private constant ever grows. Batch *write* payloads remain unbounded, deliberately out of scope. And exporting the limits over `device/info` was **dropped**, not deferred: the binding ceiling is a host-transport property the firmware cannot observe, so advertising it from firmware would be the wrong source of truth, bought with the one wire edit `validate()` cannot detect (2026-09-01 row). Issue #158. |
| 2026-09-09 | The **first** request a host process sends, at a payload in the top six bytes of the request budget. Investigation only (#180), split from #158: the first request after connect was dropped about six bytes earlier than every later one, with no explanation. The region is reachable from a supported surface only through `i2c/batch` / `spi/batch`, whose aggregate *outgoing* bytes nothing bounds; #158 keeps every single-argument write at `MAX_TRANSFER_SIZE` and therefore clear of it. | **Two independent causes, and neither is a firmware defect — the `bug(firmware)` scope on #180 was wrong.** The ceiling is on the whole request *frame*, not on any payload: 5120 (`BufStorage` RX = `MAX_TRANSFER_SIZE + 1024`, `firmware/src/main.rs`) minus 1, minus the header, minus whatever the request struct encodes around the payload. The **-1** is postcard-rpc's `EUsbWireRx::receive`: it ends a frame on a short USB packet and loops `while !window.is_empty()`, so a frame that *exactly* fills the buffer consumes the window in full 64-byte packets, never sees a short one, falls into the "ran out of space" branch, reads the terminating ZLP there and returns `ReceivedMessageTooLarge` — which `server/mod.rs` maps to `continue`, so no reply is ever sent and the caller can only time out. Smaller multiples of 64 are fine, because window remains and the ZLP ends the frame normally. The **6** is host-side and has nothing to do with the firmware: `HostClient` starts at `VarKeyKind::Key8` and adopts the server's `Key2` only after it has **received a reply** (`host_client/mod.rs:202`, `:424`, `:431` — the only three sites that touch `kkind`), so the header is 13 bytes until the first reply and 7 after. **The issue's own hint was false and saying so matters**: there is no 8-byte header anywhere in this protocol — the request header is 13 or 7 and the response header is 7 (#179) — the 8 is a *key*, and it disappears entirely once the link is warm. Both candidates #180 named are ruled out. nusb/postcard-rpc OUT chunking: `NusbWireTx::send_inner` submits the frame as one bulk-out plus a ZLP when `len % mps == 0`, with no chunking and no first-transfer special case, and a hand-built raw-nusb probe that bypasses postcard-rpc's TX entirely reproduces the identical edge. `BufStorage` not available until first use: ruled out twice, once by a discriminator (after a 5105-byte cold request has been received *and rejected*, so the buffer has demonstrably been filled, a 5110-byte request still fails, and only a *reply* changes that) and once by an A/B in which two fresh processes send the same 5110-byte payload as their first and only frame, differing only in key width — the 2-byte-key frame is answered in 4 ms. **The firmware has no cold/warm state at all.** | Documented, deliberately not fixed: `book/src/internals/wire-protocol.md` gains the term-by-term derivation, `interfaces/batching.md` gains the unenforced send-side row and the measured batch edge, `appendix/troubleshooting.md` gains "a large request times out instead of returning `BufferTooLong`". No code change, no constant, no wire or schema change. Enlarging the buffer or special-casing the first request was explicitly out of scope, and a `MAX_REQUEST_FRAME` constant belongs with the consumer that will enforce it. **Hardware-measured 2026-09-09 on board `49742081C885AC69` (hw-rev2, firmware `firmware-v0.11.0-79-gc3c6a3e07bec`), Linux/nusb, one fresh process per cold point.** Sixteen boundaries, every one predicted before it was measured by the same five-term model: `onewire/write` cold 5104/5105 and warm 5110/5111 (both monotone across 5096..5112 and 5102..5118); `onewire/write-pullup` at three `pullup_duration_ms` varint widths, cold 5103/5102/5101 and warm 5109/5108/5107, which pins the body-encoding term; hand-built frames at key widths 8/4/2/1, whose payload edges 5104/5108/5110/5111 all land on the same frame length 5119 accepted, 5120 dropped; and `pico-de-gallo-lib`'s real `i2c_batch`, cold 5099/5100 and warm 5105/5106. **This reproduces on Linux exactly as on Windows/WinUSB, so it is not platform-specific**, closing #180's third acceptance box. "Warm" means a *reply* was received, not that a request was sent, and does not drift with the warm-up's size (1, 64, 512, 4096 and 5104-byte warm-ups all give 5110). **Three traps.** A probe that takes the next IN transfer rather than matching the sequence number silently pairs a late reply with the wrong request — a sub-`MAX_TRANSFER_SIZE` 1-Wire write really does drive the bus for seconds, and one late arrival shifted every subsequent verdict by one, producing a frame-length sweep that contradicted the model until the pairing was fixed. `validate()` warms the client, so any cold measurement has to avoid it — the CLI and MCP validate on every call, so only a bare `pico-de-gallo-lib` / `gallo_init` / `open` user ever sees the cold edge. And the `-1` term is **static analysis, not measurement**: it is an unambiguous read of `receive()`'s control flow, and a positive-controlled test showed no stray frame leaks back after a refused 5120-byte frame (the control, a deliberate `Key1` frame, produced its `KeyTooSmall` reply immediately), but distinguishing "exact-fill refused" from "window one byte short" by measurement alone needs a firmware whose RX buffer is not a multiple of 64, which needs a reflash, which needs physical BOOTSEL. The batch reachability found along the way is filed separately, not folded in here. Issue #180. |
| 2026-09-09 | `i2c/batch` or `spi/batch` whose aggregate **outgoing** bytes build a request frame over 5119 bytes. Reachable from every host surface including the `gallo` CLI; the send-side mirror of the same day's #179 row, and the one door #180 left open. Nothing bounded a batch's outgoing bytes at all: `check_i2c_batch_ops` walked for zero-length writes and then checked only the *read* aggregate, `check_spi_batch_ops` only the read/transfer aggregate, and `encode_i2c_batch_ops` documented the gap rather than closing it. | **A diagnosability defect, not a corruption one** — the batch never executes, so no write reaches the bus, no chip-select edge is driven, and the board stays fully responsive. But it is the *worst-diagnosed* of the three, because the failure is silence: postcard-rpc's `receive()` discards the frame and `server/mod.rs` maps `ReceivedMessageTooLarge` to `continue`, so nothing is sent back and the caller can only time out. **Unlike #179 and #158, the firmware cannot help and cannot be made to** — it never receives the frame — which makes this the one ceiling with no device-side counterpart. On `spi/batch` it presented far worse than on `i2c/batch`: `spi_batch_delay_budget_ms` returns 0 for a batch with no `DelayNs`, and `bounded_for(0)` selects `MAX_HANDLER_TIMEOUT_MS`, so the caller waited **30 minutes** rather than 5 seconds. **Two traps.** A single batch `Write` of 5098 bytes was accepted where a plain `i2c_write` of 4097 is refused — not a bug: a batch write streams out of the received frame and never enters the firmware's `MAX_TRANSFER_SIZE` scratch buffer, so bounding it would remove working capability, and the book documents "no per-operation limit". It was left unbounded deliberately. And **a board-attached test cannot reach the true edge at boundary+1**, because the suite's `board()` calls `validate()`: the client is warm, its header is six bytes narrower, and a fixture one byte past the *cold* bound still arrives. Written blind, two such tests would have looked like they proved the refusal; the control arm caught it, and they now overshoot by more than the six-byte key-width difference, with a separate test pinning the conservative band. | `MAX_REQUEST_FRAME` (5119) added to `pico-de-gallo-internal`, derived term by term as `FIRMWARE_PACKET_BUFFER` (5120, now also exported and used to declare the firmware's `BufStorage` so the two cannot drift) less the one byte an exactly-filling frame cannot use. `i2c_batch_request_frame_len` / `spi_batch_request_frame_len` mirror the existing `*_response_len` helpers and are pinned against real `postcard::to_allocvec` encodings across the varint boundaries; `REQUEST_HEADER_LEN_MAX` (13) is pinned against `VarHeader::write_to_slice`. `pico-de-gallo-lib` gains `request_frame_is_undeliverable`, wired into both `check_*_batch_ops` per the #136 extracted-policy pattern, reporting `BufferTooLong` with `failed_op = 0`. **The bound uses the cold (13-byte) header on purpose**, because `HostClient::kkind` is private and a caller cannot ask; a warm connection is refused up to six bytes early, which is the price of an answer that does not depend on what the process did beforehand. `gallo-mcp` validates locally so an agent gets the size, limit, overshoot and remedy; the FFI gained `GALLO_MAX_REQUEST_FRAME`, **verified present in `pico_de_gallo.h`** (§8). No wire change, no schema bump, no version bump, no new `Status` — `BufferTooLong` already existed on both batch error types. Zephyr uses neither batch endpoint, so no consumer change. **Hardware-verified 2026-09-09 on board `49742081C885AC69` (hw-rev2, TMP102 at `0x48`, nothing at `0x50`, GPIO 0 as the SPI chip-select witness).** A/B between control firmware `firmware-v0.11.0-88-gfcb38de3a1ff` and the branch image (`-dirty`; `.text` byte-identical, so the firmware edit is a pure refactor), with the **host** guard stubbed out for the control arm because the firmware alone can never refuse. Control: cold `i2c_batch` 5119 -> `NoAcknowledge` in 5 ms and 5120 -> `Timeout` 5001 ms, monotone across 5096..5107; eight writes of 634 accepted and 635 dropped; `spi_batch` at the warm edge still running after 90 s and killed. Fixed: all 17 board-attached tests pass in 0.44 s, including the three #158 tests that **had never been executed** before this run. Both directions of an off-by-one in the predicate were mutation-tested, and the FFI `const` assertion was confirmed to be a build failure when violated. **Limits.** The ceiling is a property of the firmware's receive buffer, so a host built against a firmware with a different buffer would be wrong — they travel together per §6.5. Individual batch operations remain unbounded by `MAX_TRANSFER_SIZE`, deliberately. And the SPI control arm was bounded externally at 90 s rather than allowed to run the full 30 minutes, so "30 minutes" is derived from `bounded_for(0)` and not observed to completion. Issue #186. |

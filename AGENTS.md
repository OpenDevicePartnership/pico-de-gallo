...
| 2026-09-02 | Planned reproduction of the historical zero-length-I2C dispatcher wedge through the `wedge-test` firmware hatch after temporarily removing a host-side empty-write guard. **[PENDING HARDWARE — Task 8: trigger route and flashed image identity]** | **[PENDING HARDWARE — Task 8: wedge duration, endpoints attempted and timeout results]**. The designed recovery bound is `DEFAULT_DISPATCH_BUDGET + SUPERVISOR_POLL`; this is not a measurement. **[PENDING HARDWARE — Task 8: measured reset latency, RTT expiry/breadcrumb lines, USB re-enumeration, false-positive checks, and debugger-discontinuity result]**. | Replaced the unconditional `watchdog_feeder_task` with `watchdog_supervisor_task`. `WatchedRx` arms a dispatch slot after a frame arrives; `WatchedTx` tracks aggregate TX progress; slow handlers declare budgets; caller timeouts clamp at 30 minutes; and expiry records a scratch-register breadcrumb before `trigger_reset()`. **[PENDING HARDWARE — Task 8: acceptance and mutation-control verdict]**. Three limits remain: a wedge inside `receive()` is indistinguishable from legitimate idle; the TX slot has no hardware trigger and rests on inspection; and its shared slot measures aggregate, not per-sender, progress, so one permanently starved sender is masked while another completes at least once per 60-second `TX_BUDGET`. Issue #157. **THIS ROW IS INCOMPLETE AND MUST NOT MERGE UNTIL EVERY TASK 8 PLACEHOLDER IS REPLACED WITH OBSERVED RESULTS.** |

...
About **687 unit tests + 8 doctests** across the host workspace,
measured 2026-09-09:

| Crate                    | Passing | `#[ignore]`d |
|--------------------------|---------|--------------|
| `pico-de-gallo-internal` | 175     | 0            |
| `pico-de-gallo-ffi`      | 136     | 0            |
| `pico-de-gallo-lib`      | 127     | 11           |
| `gallo-mcp`              | 119     | 7            |
| `gallo`                  | 75      | 0            |
| `pico-de-gallo-hal`      | 46      | 0            |
| `pyco-de-gallo`          | 9       | 0            |
...
All 18 ignored tests are board-attached and therefore never run in CI:
...
- The 11 in `pico-de-gallo-lib` are the #135 zero-length-write
  regression tests, the #179 batch-ceiling tests, and the #158
  per-call-ceiling tests. They need one board, and
  `empty_batch_write_never_reaches_the_bus` and
  ...
  **The three #158 tests have never been executed** — no board was
  available when they were written, and they say so in their module
  comment. Treat a failure in one of them as equally likely to be a bug
  in the test until someone has run the A/B.

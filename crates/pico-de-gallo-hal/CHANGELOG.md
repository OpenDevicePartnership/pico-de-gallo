...
- **Breaking:** all eight `*HalError` enums gained a `Timeout(Duration)`
  ...
  `embedded-hal` has no timeout `ErrorKind`, so `kind()` reports
  `Other`. The concrete variant is preserved for callers that inspect the
  error directly, which is the only place the distinction can be made.

### Fixed

- HAL operations inherit the host-side call bound, so a device that never
  answers now surfaces an error instead of blocking the caller forever.
## [0.8.0] — 2026-09-01

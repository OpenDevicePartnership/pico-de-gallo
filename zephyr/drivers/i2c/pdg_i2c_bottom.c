 ...
 */
__attribute__((weak)) int pdg_i2c_bottom_set_config(void *ctx, uint8_t frequency)
{
	return pdg_common_status_to_errno(gallo_i2c_set_config((const struct PicoDeGallo *)ctx, frequency));
}

__attribute__((weak)) int pdg_i2c_bottom_write(void *ctx, uint16_t addr, const uint8_t *buf, size_t len)
{
	return pdg_common_status_to_errno(gallo_i2c_write((const struct PicoDeGallo *)ctx, (uint8_t)addr, buf, len));
}

__attribute__((weak)) int pdg_i2c_bottom_read(void *ctx, uint16_t addr, uint8_t *buf, size_t len)
{
	return pdg_common_status_to_errno(gallo_i2c_read((const struct PicoDeGallo *)ctx, (uint8_t)addr, buf, len));
}

__attribute__((weak)) int pdg_i2c_bottom_write_read(void *ctx, uint16_t addr, const uint8_t *tx, size_t txlen, uint8_t *rx, size_t rxlen)
{
